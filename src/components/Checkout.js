...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      //console.log("clicked", token, newAddress);
  ...
  //Display address if user is logged in else redirect to products page
  useEffect(() => {
    if (token) {
      getAddresses(token);
    } else {
    ...
    <>
      ...
      <Grid container>
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          ...
          <Box backgroundColor="#ffffff" px="1rem" py="1rem" borderRadius="4px" margin="0.5rem">
            <Typography color="#3C3C3C" variant="h5" my="1rem" fontWeight="600">
              Order Details
            </Typography>
